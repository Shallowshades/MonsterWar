...
namespace engine::input {
		...
		: mSDLRenderer(renderer), mDispatcher(dispatcher)
	{
		...
		// 检测触摸设备
		int touch_count = 0;
		SDL_GetTouchDevices(&touch_count);
		mIsTouchDevice = touch_count > 0;
	...
	bool InputManager::isTouchDevice() const {
	...
	glm::vec2 InputManager::getTouchPosition() const {
		return mTouchPosition;
	}

	glm::vec2 InputManager::getTouchStartPosition() const {
		return mTouchStartPosition;
	}

	void InputManager::setUiHitTester(std::function<bool(const glm::vec2&)> tester) {
		mUiHitTester = std::move(tester);
	}

	bool InputManager::isClickConsumed() const {
		return mClickConsumed;
	}

	void InputManager::consumeNextClick() {
		mClickConsumed = true;
	}

	void InputManager::clearClickConsumed() {
	...
	void InputManager::initializeMappings(const engine::core::Config* config) {
		...
		// 触摸动作：仅注册状态，不绑定物理键/鼠标，由 FINGER 事件驱动
		for (const auto& touch_action : { "touch_begin", "touch_move", "touch_end", "touch_cancel" }) {
			if (actionsToKeyname.find(touch_action) == actionsToKeyname.end()) {
				actionsToKeyname[touch_action] = {};
			}
		}
