...
namespace engine::loader {
    ...
    protected:
        engine::loader::LevelLoader& mLevelLoader;
        engine::core::Context& mContext;
        entt::registry& mRegistry;


        // 解析游戏对象所需要的关键信息
        const nlohmann::json* mObjectJson = nullptr;                ///< @brief 来自.tmj地图文件的对象数据
        const engine::component::TileInfo* mTileInfo = nullptr;     ///< @brief 来自.tsj的瓦片数据
        int mIndex = -1;                                            ///< @brief 瓦片索引，用于计算位置（瓦片层）
