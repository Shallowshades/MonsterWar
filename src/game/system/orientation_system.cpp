...
namespace game::system {
    ...
    void OrientationSystem::updateHasTarget(entt::registry& registry) {
        ...
        for (auto entity : view_has_target) {
            ...
            const auto& target_transform = view_has_target.get<engine::component::TransformComponent>(target.mEntity);
            ...
            bool face_right = target_transform.mPosition.x > transform.mPosition.x;
            // 使用 all_of 检查是否存在指定标签 (FaceLeftTag)
            if (registry.all_of<game::defs::FaceLeftTag>(entity)) {
                sprite.mSprite.mIsFlipped = face_right;
            }
            else {
                sprite.mSprite.mIsFlipped = !face_right;
            }
    ...
    void OrientationSystem::updateBlocked(entt::registry& registry) {
        ...
        for (auto entity : view_blocked) {
            ...
            const auto& blocked_by_transform = view_blocked.get<engine::component::TransformComponent>(blocked_by.mEntity);
            ...
            bool face_right = blocked_by_transform.mPosition.x > transform.mPosition.x;
            if (registry.all_of<game::defs::FaceLeftTag>(entity)) {
                sprite.mSprite.mIsFlipped = face_right;
            }
            else {
    ...
    void OrientationSystem::updateMoving(entt::registry& registry) {
        ...
        for (auto entity : view_moving) {
            ...
            // 根据速度的 x 分量符号判断朝向
            bool face_right = velocity.mVelocity.x > 0.0f;
            if (registry.all_of<game::defs::FaceLeftTag>(entity)) {
                sprite.mSprite.mIsFlipped = face_right;
            }
            else {
