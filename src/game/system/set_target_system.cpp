...
namespace game::system {
    ...
    void SetTargetSystem::updateHasTarget(entt::registry& registry) {
        ...
        for (auto entity : view_has_target) {
            ...
            // 检查目标是否还有效
            if (!registry.valid(target.mEntity)) {
                // 如果目标实体无效，则清除目标
                registry.remove<game::component::TargetComponent>(entity);
                spdlog::info("ID: {}, 目标: ID: {}, 无效, 清除目标",
                    entt::to_integral(entity),
                    entt::to_integral(target.mEntity));
                continue;
            }
            // 检查目标是否还在攻击范围之内（检测半径 = 角色攻击范围 + 目标角色半径）
            const auto& target_transform = registry.get<engine::component::TransformComponent>(target.mEntity);
            auto range_radius = stats.mRange + game::defs::UNIT_RADIUS;
            if (engine::utils::distanceSquared(transform.mPosition, target_transform.mPosition) > range_radius * range_radius) {
                // 如果在攻击范围外，则清除目标
                registry.remove<game::component::TargetComponent>(entity);
                spdlog::info("ID: {}, 目标: ID: {}, 不在攻击范围之内, 清除目标", entt::to_integral(entity), entt::to_integral(target.mEntity));
                continue;
            }
        }
    }
    ...
    void SetTargetSystem::updateNoTargetPlayer(entt::registry& registry) {
        ...
        for (auto player_entity : view_player_no_target) {
            ...
            for (auto enemy_entity : view_enemy) {
                ...
                auto range_radius = player_stats.mRange + game::defs::UNIT_RADIUS;
                if (engine::utils::distanceSquared(player_transform.mPosition, enemy_transform.mPosition) <= range_radius * range_radius) {
                    // 如果敌人在攻击范围之内，则设置目标
    ...
    void SetTargetSystem::updateNoTargetEnemy(entt::registry& registry) {
        ...
        for (auto enemy_entity : view_enemy_no_target) {
            ...
            for (auto player_entity : view_player) {
                ...
                auto range_radius = enemy_stats.mRange + game::defs::UNIT_RADIUS;
                if (engine::utils::distanceSquared(enemy_transform.mPosition, player_transform.mPosition) <= range_radius * range_radius) {
                    // 如果玩家角色在攻击范围之内，则设置目标
    ...
    void SetTargetSystem::updateHealer(entt::registry& registry) {
        ...
        for (auto healer_entity : view_healer) {
            ...
            for (auto player_entity : view_injured_player) {
                ...
                auto range_radius = healer_stats.mRange + game::defs::UNIT_RADIUS;
                if (engine::utils::distanceSquared(healer_transform.mPosition, player_transform.mPosition) < range_radius * range_radius) {
                    // 计算血量百分比并更新最低百分比和目标角色
                    const auto& player_stats = view_injured_player.get<game::component::StatsComponent>(player_entity);
                    auto hp_percent = static_cast<float>(player_stats.mHp) / static_cast<float>(player_stats.mMaxHp);
