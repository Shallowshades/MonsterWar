...
namespace game::system {
    ...
    void HealthBarSystem::update(entt::registry& registry, engine::render::Renderer& renderer, engine::render::Camera& camera) {
        ...
        for (auto entity : view) {
            ...
            // 血量条位置 = 角色位置 + 偏移量（水平居中）
            auto position = transform.mPosition + glm::vec2(-size.x / 2.0f, game::defs::HEALTH_BAR_OFFSET_Y);

            // 根据血量百分比确定颜色
            auto health_percent = static_cast<float>(stats.mHp) / static_cast<float>(stats.mMaxHp);
