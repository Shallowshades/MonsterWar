...
namespace game::system {
    ...
    void BlockSystem::update(entt::registry& registry, entt::dispatcher& dispatcher) {
        ...
        for (auto blocked_by_entity : view_blocked_by) {
            ...
            if (!registry.valid(blocked_by_component.mEntity)) {
                spdlog::info("阻挡者: ID: {}, 无效, 移除 ID: {} 的阻挡者组件", entt::to_integral(blocked_by_component.mEntity), entt::to_integral(blocked_by_entity));
        ...
        for (auto enemy_entity : view_enemy) {
            ...
            for (auto blocker_entity : view_blocker) {
                ...
                if (engine::utils::distanceSquared(enemy_transform.mPosition, blocker_transform.mPosition) <
                    game::defs::BLOCK_RADIUS * game::defs::BLOCK_RADIUS) {
                    // 检查阻挡者是否还能阻挡
                    if (blocker_blocker.mCurrentCount >= blocker_blocker.mMaxCount) {
                        continue;   // 如果不能阻挡，则跳过
                    }
                    blocker_blocker.mCurrentCount++;                    // 增加阻挡数量
                    enemy_velocity.mVelocity = glm::vec2(0.0f, 0.0f);   // 设置敌人速度为0
                    // 给敌人添加被阻挡组件
